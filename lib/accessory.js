...
class HygrothermographCgdk2AccessoryHandler {
  ...
  setTemperature(newValue, force = false) {
    ...
    this.currentTemperatureCharacteristic.updateValue(newValue);
    this.temperatureStatusFaultCharacteristic.updateValue(this.statusFault);
  ...
  setHumidity(newValue, force = false) {
    ...
    this.currentHumidityCharacteristic.updateValue(newValue);
    this.humidityStatusFaultCharacteristic.updateValue(this.statusFault);
    this.addFakeGatoHistoryEntry();
  ...
  setBatteryLevel(newValue, force = false) {
    ...
    if (this.batteryLevelCharacteristic != null) {
      this.batteryLevelCharacteristic.updateValue(newValue);
    }
  ...
  setRSSI(newValue) {
    ...
    this.rssiCharacteristic.updateValue(newValue);
    if (this.lastUpdatedAt != null) {
      this.lastSeenCharacteristic.updateValue(this.lastUpdatedISO8601);
    }
  }
  ...
  // Checked by several getters (temperature, humidity, batteryLevel,
  // statusFault), all of which can be read multiple times per single sensor
  // update (e.g. a batch flush reads all three). Without tracking which
  // lastUpdatedAt we already warned about, a stale sensor would log this
  // warning repeatedly on every one of those reads instead of once.
  hasTimedOut() {
    if (this.timeout === 0 || this.lastUpdatedAt == null) {
      return false;
    }
    const timeoutMilliseconds = 1000 * 60 * this.timeout;
    const timedOut = this.lastUpdatedAt <= Date.now() - timeoutMilliseconds;
    if (timedOut && this.timeoutWarnedFor !== this.lastUpdatedAt) {
      this.timeoutWarnedFor = this.lastUpdatedAt;
      this.log.warn(
  ...
  getTemperatureService() {
    ...
    // Cached so setTemperature/setRSSI (called on every sensor reading) can
    // push updates directly instead of re-resolving the characteristic by
    // UUID through getCharacteristic() every time.
    this.currentTemperatureCharacteristic = temperatureService
      .getCharacteristic(Characteristic.CurrentTemperature)
      .on("get", this.onCharacteristicGetValue.bind(this, "temperature"))
      .setProps({ minValue: -10, maxValue: 60 });
    this.temperatureStatusFaultCharacteristic = temperatureService
      .getCharacteristic(Characteristic.StatusFault)
      .on("get", this.onCharacteristicGetValue.bind(this, "statusFault"));
    this.rssiCharacteristic = temperatureService
      .getCharacteristic(RSSICharacteristic)
      .on("get", this.onCharacteristicGetValue.bind(this, "rssi"));
    this.lastSeenCharacteristic = temperatureService
  ...
  getHumidityService() {
    ...
    this.currentHumidityCharacteristic = humidityService
      .getCharacteristic(Characteristic.CurrentRelativeHumidity)
      .on("get", this.onCharacteristicGetValue.bind(this, "humidity"));
    this.humidityStatusFaultCharacteristic = humidityService
      .getCharacteristic(Characteristic.StatusFault)
      .on("get", this.onCharacteristicGetValue.bind(this, "statusFault"));
  ...
  getBatteryService() {
    ...
    if (this.isBatteryLevelDisabled) {
      ...
      this.batteryLevelCharacteristic = undefined;
      return undefined;
    }
    ...
    this.batteryLevelCharacteristic = batteryService
...
module.exports = (homebridge) => {
  ...
  // HAP-NodeJS 2.x (Homebridge 2.0) dropped the `Characteristic.Formats`/
  // `Characteristic.Perms` aliases; Formats/Perms are only ever available as
  // top-level `hap` exports, on both HAP-NodeJS 1.x and 2.x.
